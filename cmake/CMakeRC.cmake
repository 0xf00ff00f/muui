...
if(COMMAND cmrc_add_resource_library)
  if(NOT DEFINED _CMRC_VERSION OR NOT (_version STREQUAL _CMRC_VERSION))
    message(
      WARNING "More than one CMakeRC version has been included in this project."
    )
  endif()
...
set(_CMRC_VERSION
    "${_version}"
    CACHE INTERNAL "CMakeRC version. Used for checking for conflicts")

set(_CMRC_SCRIPT
    "${CMAKE_CURRENT_LIST_FILE}"
    CACHE INTERNAL "Path to CMakeRC script")
...
set(CMRC_INCLUDE_DIR
    "${_inc_dir}"
    CACHE INTERNAL "Directory for CMakeRC include files")
...
set(cmrc_hpp
    "${CMRC_INCLUDE_DIR}/cmrc/cmrc.hpp"
    CACHE INTERNAL "")
set(_generate 1)
if(EXISTS "${cmrc_hpp}")
...
target_include_directories(cmrc-base
                           INTERFACE $<BUILD_INTERFACE:${CMRC_INCLUDE_DIR}>)
...
function(cmrc_add_resource_library name)
  ...
  if(NOT DEFINED ARG_NAMESPACE)
    ...
    if(NOT name MATCHES "${ns_re}")
      message(
        SEND_ERROR
          "Library name is not a valid namespace. Specify the NAMESPACE argument"
      )
    endif()
    ...
    if(NOT ARG_NAMESPACE MATCHES "${ns_re}")
      message(
        SEND_ERROR
          "NAMESPACE for ${name} is not a valid C++ namespace identifier (${ARG_NAMESPACE})"
      )
    endif()
  endif()
  set(libname "${name}")
  ...
  elseif(NOT "${ARG_TYPE}" MATCHES "^(STATIC|OBJECT)$")
    message(
      SEND_ERROR
        "${ARG_TYPE} is not a valid TYPE (STATIC and OBJECT are acceptable)")
    set(ARG_TYPE STATIC)
  endif()
  ...
  string(
    CONFIGURE
      ...
      cpp_content
    @ONLY)
  get_filename_component(libdir "${CMAKE_CURRENT_BINARY_DIR}/__cmrc_${name}"
                         ABSOLUTE)
...
function(_cmrc_register_dirs name dirpath)
  ...
  set_property(
    TARGET "${name}"
    APPEND
    PROPERTY _CMRC_REGISTERED_DIRS "${dirpath}")
...
function(cmrc_add_resources name)
  ...
  if(NOT TARGET ${name} OR NOT is_reslib)
    message(
      SEND_ERROR
        "cmrc_add_resources called on target '${name}' which is not an existing resource library"
    )
    return()
  endif()
  ...
  if(reldir MATCHES "^\\.\\.")
    message(
      SEND_ERROR
        "Cannot call cmrc_add_resources in a parent directory from the resource library target"
    )
    return()
  endif()
  ...
  foreach(input IN LISTS ARG_UNPARSED_ARGUMENTS)
    ...
    # Generate a filename based on the input filename that we can put in the
    ...
    if(relpath MATCHES "^\\.\\.")
      ...
      message(
        SEND_ERROR
          "Cannot add file '${input}': File must be in a subdirectory of ${ARG_WHENCE}"
      )
      continue()
    endif()
    ...
    get_filename_component(
      abs_out "${libdir}/intermediate/${ARG_PREFIX}${relpath}.cpp" ABSOLUTE)
    ...
    set_property(
      TARGET ${name}
      APPEND
      PROPERTY CMRC_EXTERN_DECLS "// Pointers to ${input}"
...
function(_cmrc_generate_intermediate_cpp lib_ns symbol outfile infile)
  add_custom_command(
    ...
    COMMAND
      "${CMAKE_COMMAND}" -D_CMRC_GENERATE_MODE=TRUE -DNAMESPACE=${lib_ns}
      -DSYMBOL=${symbol} "-DINPUT_FILE=${infile}" "-DOUTPUT_FILE=${outfile}" -P
      "${_CMRC_SCRIPT}"
